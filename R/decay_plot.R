...
function (geneID,
          ...
          desc.width = 55)
{
if (any(!geneID %in% rownames(mod.results))) {
  stop(paste0("geneID:", geneID, " not found in the dataset."),
       call. = FALSE)
}
if (any(!treatments %in% gsub("alpha_", "", colnames(mod.results)[grep("alpha_",
                                                                       colnames(mod.results)[1:4])]))) {
...
if (any(what %in% "models"))
  mod <- mod.results[geneID, "mod"] else mod <- NA
if (any(what %in% "models"))
  A_grp <- mod.results[geneID, "alpha_subgroup"] else ""
if (is.na(treatments[1]))
  treatments <- unique(DATA$treatment) else treatments <- treatments[treatments %in% unique(DATA$treatment)]
if (is.na(colors[1]))
  colors <- grDevices::rainbow(length(treatments), alpha = 1)
if (any(is.na(xticks)))
...
if (any(what %in% "Desc")) {
  p <- p + ggplot2::annotate(geom = "text", x = xlim[2]/2,
                             y = ylim[2], label = wrapper(paste0(geneID, if (any(what %in%
                                                                                 "models")) {
                               paste0(" - model ", mod)
                             }
                             else {
                               ""
                             }, paste0(" - ", gdesc[geneID])), desc.width), vjust = 1,
                             hjust = 0.5, color = grDevices::gray(0.4), size = alphaSZ *
                               0.4, family = c("mono"), fontface = "plain",
                             angle = 0)
}
else {
  ...
  p <- p + ggplot2::geom_text(parse = TRUE,
                              data = data.frame(treatment = treatments,
                                                text = sapply(treatments, function(g) {
                                                  paste0("alpha==", as.character(round(mod.results[geneID,
                                                                                                   paste0("alpha_", g)], 4)), "~beta==", as.character(round(mod.results[geneID,
                                                                                                                                                                        paste0("beta_", g)], 4)))
                                                }), x = rep(xlim[2], length(treatments)), y = if (any(mod.results[geneID,
                                                                                                                  (length(unique(DATA$treatment)) + 1):(length(unique(DATA$treatment)) *
                                                                                                                                                        2)] == 0)) {
                                                  sapply(treatments, function(g) {
                                                    c(max(fun_exp(xlim[2], unlist(mod.results[geneID,
                                                                                              paste0("alpha_", g)])) - 0.2, 0))
                                                  })
                                                }
                                                else {
                                                  sapply(treatments, function(g) {
                                                    c(max(dExp(xlim[2], unlist(mod.results[geneID,
                                                                                           paste0(c("alpha_", "beta_"), g)])) - 0.2,
                                                          0))
                                                  })
                                                }),
                              mapping = ggplot2::aes(label = text, x = x, y = y,
                                                     color = treatment),
                              vjust = -0.5, hjust = 1, size = (alphaSZ/1.3) *
                                1.5, alpha = 0.75)
}
if (any(what %in% "meanSE")) {
  p <- p + ggplot2::stat_summary(ggplot2::aes(x = t.decay,
                                              y = value, color = treatment), fun = mean, geom = "line",
                                 linewidth = if (any(what %in% "models")) {
                                   0.35
                                 }
                                 else {
                                   1
    ...
    ggplot2::stat_summary(ggplot2::aes(x = t.decay,
                                       y = value, color = treatment), fun.data = ggplot2::mean_se,
                          geom = "errorbar", linewidth = 0.35, alpha = 0.6)
}
if (any(what %in% "reps")) {
...
p <- p + ggplot2::scale_color_manual("", breaks = names(colors),
                                     values = colors, labels = gsub("\\.", " ", treatments)) +
  ggplot2::coord_cartesian(xlim = xlim, ylim = ylim) +
  ggplot2::scale_y_continuous(breaks = yticks) + ggplot2::scale_x_continuous(breaks = xticks) +
  ggplot2::ylab("relative abundance") + ggplot2::xlab("time (min)") +
