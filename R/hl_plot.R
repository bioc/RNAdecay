...
)
{
  ...
  hls = round(log(2)/df_decay_rates[geneID, treatment.cols],
              1)
  if (all(is.na(arrow_colors))) {
    arrow_colors <- c("#88CCEE", "#CC6677", "#DDCC77", "#117733",
                      "#332288", "#AA4499", "#44AA99", "999933", "#882255",
                      "#661100", "#6699CC")[seq_along(hl_treatment)]
    names(arrow_colors) = hl_treatment
  }
  if (!hl_dist_treatment %in% hl_treatment) {
    stop("'hl_dist_treatment' must be one of 'hl_treatment'.")
  }
  if (length(geneID) > 1) {
  ...
  if (is.null(names(arrow_colors)) | !all(names(arrow_colors) %in%
                                          hl_treatment)) {
  ...
  labs <- sapply(hl_treatment, function(x) {
    ...
    if (hl > max(x_breaks)) {
      ...
      paste0("paste(\"", x, " \",italic(t)[\"1/2\"] == \"",
             if (hl < 120) {
               paste0(as.character(round(hl)), " min")
             }
             else {
               paste0(round(hl/60, 1), " h")
             }, "\")")
    }
  })
  hl_dist <- df_decay_rates[df_decay_rates[, treatment.cols[hl_dist_treatment]] >
                              4.8e-05, treatment.cols[hl_dist_treatment]]
  quants <- round(stats::quantile(log(2)/hl_dist))
  peak <- max(table(cut(log10(log(2)/hl_dist), include.lowest = TRUE,
                        breaks = c(seq(from = log10(x_limits[1]), to = x.max <- if (max(x_breaks) <
                                                                                    max(log(2)/hl_dist)) {
                          log10(max(x_limits))
                        } else {
                          max(log10(log(2)/hl_dist))
                        }, by = 0.02)))))
  ...
  arrowhead_length_in <- grDevices::dev.size(units = "in")[2] *
    0.04 * 0.85^seq_along(hl_treatment)[1] # [1] was added here 2025-10-24 because ggplot 4.0.0 broke the prior functionality.
  # previously the arrowhead lengths could all be different, but now the arrow() function length argument only takes
  # a single length
  names(arrowhead_length_in) <- hl_treatment[1] # [1] added 2025-10-24 see above comment
  p <- ggplot2::ggplot(df_decay_rates[df_decay_rates[, treatment.cols[hl_dist_treatment]] >
                                        4.8e-05, ])
  p <- p + ggplot2::geom_histogram(data = data.frame(hl_dist = hl_dist),
                                   mapping = ggplot2::aes(x = log(2)/hl_dist), binwidth = 0.02,
                                   fill = grDevices::gray(0.8)) +
    ggplot2::geom_vline(xintercept = quants[2],
                        color = "white", linetype = "dashed", linewidth = 0.25) +
    ggplot2::geom_vline(xintercept = quants[3], color = "white",
                        linetype = "dashed", linewidth = 0.25) + ggplot2::geom_vline(xintercept = quants[4],
                                                                                     color = "white", linetype = "dashed", linewidth = 0.25) +
    ...
    ggplot2::annotate(geom = "text",
                      ...
                      hjust = 0.575, vjust = -0.2, color = grDevices::gray(0.65),
  ...
  if (arrow_lab_loc == "plot") {
    for (i in hl_treatment) {
      ...
      p <- p + ggplot2::geom_segment(data = data.frame(x = if (hl >
                                                               x_limits[2]) {
        x_limits[2]
      }
      else {
        hl
      }, y = arrow_lengths[i]), mapping = ggplot2::aes(x = x,
                                                       y = y, xend = x, yend = 0),
      arrow = ggplot2::arrow(angle = 18,
                             length = grid::unit(arrowhead_length_in, "in"),
                             type = "closed"), color = arrow_colors[i],
      linewidth = arrow_thickness[i],
      lineend = "butt", linejoin = "mitre")
      p <- p + ggplot2::annotate(parse = TRUE,
                                 geom = "text",
                                 x = if (hl > max(x_limits[2])) {
                                   max(x_limits[2])
                                 } else {
                                   hl
                                 }, y = arrow_label_heights[i],
                                 label = labs[i],
                                 hjust = if (hl > 103.4) {
                                   0.9
                                 }
                                   ...
                                   0.15
                                 }, vjust = 0.5, color = arrow_colors[i], size = 72.27 *
                                   grDevices::dev.size(units = "in")[2]/75)
    }
  }
  if (arrow_lab_loc == "key") {
    hl <- log(2)/df_decay_rates[geneID, treatment.cols]
    p <- p + ggplot2::geom_segment(data = data.frame(x = unlist(hl),
                                                     y = arrow_lengths[hl_treatment], labs = hl_treatment),
                                   mapping = ggplot2::aes(x = x, y = y, xend = x, yend = 0,
                                                          color = labs, linewidth = labs),
                                   arrow = ggplot2::arrow(angle = 18,
                                                          length = grid::unit(arrowhead_length_in, "in"),
                                                          type = "closed"),
                                   lineend = "butt", linejoin = "mitre") +
      ggplot2::scale_color_manual(NULL, values = arrow_colors,
                                  breaks = hl_treatment, labels = (scales::parse_format())(labs[hl_treatment])) +
      ggplot2::scale_discrete_manual(aesthetic = "linewidth", values = arrow_thickness) +
      ggplot2::guides(size = "none")
  }
  p <- p + ggplot2::annotate(geom = "text", label = if (gene_symbol !=
                                                        "") {
    paste0(geneID, "/", gene_symbol)
  }
  else {
    geneID
  }, x = 10^x.max, y = peak * 1.25, hjust = 1, vjust = 1, color = "black",
  size = 72.27 * grDevices::dev.size(units = "in")[2]/50) +
    ggplot2::xlab(expression(italic(t)["1/2"])) + ggplot2::coord_cartesian(xlim = x_limits,
                                                                           ylim = c(0, peak * 1.25)) + ggplot2::theme_classic() +
